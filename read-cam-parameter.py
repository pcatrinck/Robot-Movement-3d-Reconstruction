...
RTcw = [R0,T0,R1,T1,R2,T2,R3,T3] # from camera to world
RTwc = []                        # from world to camera


def fromWorldToCamera(R,T):
    M = R@T
    Minv = np.linalg.inv(M)
    return Minv

for i in range(0, len(RTcw), 2):
    Minv = fromWorldToCamera(RTcw[i],RTcw[i+1])
    fromWorldToCamera.append(Minv)
